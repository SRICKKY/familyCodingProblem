...
class Family:
    """
    Handles a family members and gives information about the members through
    functions
    """
    members = []
    ...
    def __init__(self, leader_man):
        """
        fundamental person for a family(right now its king), with that Man,
    ...
    def marriage_of_a_family_member(self, member, spouse):
        ...
        :param spouse: new person who is going to get married to family member
        and going to added in family
        """
    ...
    def connect_new_born_to_parent(child, parents):
        """
        Connects a child(a Person object) to its parents(adding into sons and
        daughters list)
        :param child: Child who is going to get added into list of sons or
        daughters
    ...
    def get_brother_in_laws(person):
        ...
        if person is None:
            return []
        brother_in_laws = []
        # brother-in_laws are spouse's brothers and husbands of
        # siblings(girl siblings)
    ...
    def get_uncles(person, parent):
        """
        :param person: person an Instance of Person class
        :param parent: Either mother or father instance, so according to that
        either we will get paternal aunt to maternal aunts
        :return: a list of person objects of uncles of given person(either
    ...
    def get_aunt(person, parent):
        """
        :param person: person an Instance of Person class, that given Family's
        relatives are going to be retrieved
        :param parent: Either mother or father instance, so according to that
        either we will get paternal aunt to maternal aunts
        :return: a list of person objects of aunts of given person(either
        maternal or paternal)
        """
        if person is None:
            return []
    ...
    def get_cousins(person):
        """
        :param person: person an Instance of Person class, that given Familys
        relatives are going to be retrieved
        ...
        cousins = []
        mother_sibling = Person.get_brothers(mother) + Person.get_sisters(
            mother)
        for sibling in mother_sibling:
            cousins.extend(Person.get_children(sibling))
        father_sibling = Person.get_brothers(father) + Person.get_sisters(
            father)
